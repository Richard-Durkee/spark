...
@ExpressionDescription(
  usage = """_FUNC_(expr1, expr2, expr3) - If `expr1` evaluates to true, then returns `expr2`; otherwise returns `expr3`.
    _FUNC_(expr1, expr2) - If `expr1` evaluates to true, then returns `expr2`; otherwise null is returned.""",
  examples = """
    Examples:
...
case class If(predicate: Expression, trueValue: Expression, falseValue: Expression)
  ...
  def this(predicate: Expression, trueValue: Expression) =
    this(predicate, trueValue, Literal.create(null, trueValue.dataType))
